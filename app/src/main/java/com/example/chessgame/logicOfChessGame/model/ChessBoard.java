...
import com.example.chessgame.logicOfChessGame.entity.Pawn;
...
public class ChessBoard {
    private final ChessCell[][] board = new ChessCell[8][8];
    public ChessBoard(){
        for (int y = 0; y < 8;y++){
            for (int x = 0; x < 8;x++){
                if ((y+x)%2 != 0){
    ...
    public void movePiece(ChessMove move){
        board[move.getToCellY()][move.getToCellX()].movePiecesFrom(board[move.getFromCellY()][move.getFromCellX()]);
    }
}
